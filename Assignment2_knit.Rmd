...
```{r Data Clean up from Asgnt1, include=FALSE}
...
gr<-xgb.plot.tree(model=xgb_lsM1, trees=1, render=FALSE)
export_graph(gr, 'tree_loanStatus.pdf', width=1500, height=1900)
...
#Random Forest Model from Asgnt1
rfModel2 <- ranger(loan_status ~., data=lcdfTrn %>%  select(-c('actualTerm', 'actualReturn', 'total_pymnt','annRet', 'issue_d')), num.trees =500, probability = TRUE, min.node.size = 50, importance='permutation', max.depth = 15)


#ROC for Decision Tree
perfROC_DT=performance(prediction(predict(lcDT1p,lcdfTst, type="prob")[,2], lcdfTst$loan_status), "tpr", "fpr")
#ROC for Random Forest
perfROC_RF=performance(prediction(predict(rfModel2,lcdfTst)$predictions[,2], lcdfTst$loan_status,label.ordering=c("Charged Off","Fully Paid")), "tpr", "fpr")
#ROC for xgBoost model
xpredTst<-predict(xgb_lsM1, dxTst,type="prob")
pred_xgb_lsM1=performance(prediction(xpredTst, lcdfTst$loan_status,label.ordering=c("Charged Off","Fully Paid")), "tpr", "fpr")

#ROC graph for all 3 models together
plot(perfROC_DT, col='red')
plot(perfROC_RF, col='blue', add=TRUE)
plot(pred_xgb_lsM1, col='green', add=TRUE)
legend('bottomright', c('Pruned DecisionTree Model', 'RandomForest Model', 'XGBoost Model'), lty=1, col=c('red', 'blue', 'green'))  


#AUC value for DT
predDT=prediction(predict(lcDT1p,lcdfTst, type="prob")[,2], lcdfTst$loan_status )
aucPerfDT=performance(predDT, "auc")
sprintf("AUC: %f", aucPerfDT@y.values)
# AUC values for Random Forest Model
scoreTst2 <- predict(rfModel2,lcdfTst)
predRF=prediction(scoreTst2$predictions[, "Fully Paid"], lcdfTst$loan_status, label.ordering = c("Charged Off","Fully Paid" ))
aucPerfRF=performance(predRF, "auc")
sprintf("AUC: %f", aucPerfRF@y.values)


#lift curve for the DT, random forest and XGboost
perfLift_DT=performance(prediction(predict(lcDT1p,lcdfTst, type="prob")[,"Fully Paid"], lcdfTst$loan_status ,label.ordering=c("Charged Off","Fully Paid")), "lift", "rpp")
perfLift_RF=performance(prediction(predict(rfModel2,lcdfTst)$predictions[,"Fully Paid"],lcdfTst$loan_status,label.ordering=c("Charged Off","Fully Paid")), "lift", "rpp")
perfLift_XGB=performance(prediction(xpredTst, lcdfTst$loan_status,label.ordering=c("Charged Off","Fully Paid")), "lift", "rpp")

#Comparing Lift Curves of three  models
plot(perfLift_DT, col="red")
plot(perfLift_RF, add=TRUE, col="blue")
plot(perfLift_XGB, add=TRUE, col="green")
legend('bottomright', c('Pruned DecisionTree Model', 'RandomForest Model', 'XGBoost Model'), lty=1, col=c('red', 'blue', 'green'))
...
#Comparison of profit curves for DT, Random Forest and XGBoost
plot(prPerfRF$cumProfit,col='blue',lwd = 0.5)
lines(prPerf$cumProfit,col='red',lwd = 0.5)
lines(prPerfXGB$cumProfit, col='green',lwd = 0.5)
legend('bottomright', c('cum profit RF','cum profit DT', 'Cum profit XGB'), lty=1, col=c('blue','red', 'green'))

```


```{r Question 1 : Decile Lift Performance for XGB model}
...
fnDecileLiftsPerformance_defaults  <- function( scores, dat) {  #score is for loan_status=='Charged Off'
  ...
  decPerf<-  decPerf  %>% group_by(decile) %>% summarise ( 
    ...
    totE=sum(grade=="E"),totF=sum(grade=="F") , totG=sum(grade=='G'))
...
#decile lift performance by defaults
scoreXGB <- predict(xgb_lsM1,dxTst)
fnDecileLiftsPerformance_defaults( predict(xgb_lsM1, dxTst), lcdfTst  ) 
# Returns performance by decile
fnDecileReturnsPerformance( predict(xgb_lsM1, dxTst, type="response"), lcdfTst  ) #on test data
...
#encoded the dependent var here to make sure that 1 is for "Fully Paid"
yTrn<-factor(if_else(lcdfTrn$loan_status=="Fully Paid", '1', '0') )
#predictor variable
xDTrn<-lcdfTrn %>% select(-loan_status, -actualTerm, -actualReturn, -total_pymnt, -annRet, -issue_d)

# cross validation on generalized linear model 
# family: “binomial” for a binary dependent variable


## Model 1 with defaults ## 
...
glmls_cv$lambda.1se
plot(glmls_cv)
#The vertical lines show the locations of λmin and λ1se
#The numbers across the top are the number of nonzero coefficient estimates
#coefficients values for the variables
...
#graphs for the default model 1
plot(glmls_cv$glmnet.fit)
plot(glmls_cv$glmnet.fit, xvar= "lambda")
plot(glmls_cv$glmnet.fit, xvar= "dev")
#predictions for the cv model (Model 1 with defaults)
glmls_cv_pred=predict ( glmls_cv,data.matrix(xDTrn), s="lambda.min",  type="response"  )
predsauc_cv <- prediction(glmls_cv_pred, lcdfTrn$loan_status, label.ordering = c("Charged Off", "Fully Paid"))
aucPerf_cv <- performance(predsauc_cv, "auc")
aucPerf_cv@y.values


## Model 2 with type.measure = auc## 
# cross validation with type.measure = auc :  gives area under the ROC curve
...
glmls_cv_auc$cvm
#to get the 'loss' value at lambda == lambda.1se
glmls_cv_auc$cvm [ which(glmls_cv_auc$lambda == glmls_cv_auc$lambda.1se) ]
#predictions
glmls_cv_auc_pred=predict ( glmls_cv_auc,data.matrix(xDTrn), s="lambda.min",  type="response"  )
...
glmls_1_pred <- factor(glmls_1_pred, levels=c(1,0))
yTst <- factor(yTst, levels=c(1,0))                         
caret::confusionMatrix(glmls_1_pred,yTst, positive="1") 
# Confusion matrix for Ridge model with unbalanced data
glm_ridge <- glmnet(data.matrix(xDTst), yTst, family="binomial", type.measure="deviance", alpha = 0,
                  lambda = glmls_cv_ridge$lambda.min )
glm_ridge_pred <- predict(glm_ridge, data.matrix(xDTst),s="lambda.min",type="class")
glm_ridge_pred <- factor(glm_ridge_pred, levels=c(1,0))
yTst <- factor(yTst, levels=c(1,0))                         
caret::confusionMatrix (glm_ridge_pred,yTst, positive="1") 
...
###GLM###
# GLM Lasso  model for actual returns on training data
# training and test data sets
xD<-lcdfTrn %>% select(-loan_status, -actualTerm, -actualReturn, -total_pymnt, -annRet, -issue_d)
xD_1<-lcdfTst %>% select(-loan_status, -actualTerm, -actualReturn, -total_pymnt, -annRet, -issue_d)
